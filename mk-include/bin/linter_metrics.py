...
def parse_linter_metrics(input, api_name):
    ...
    metrics = []
    for rule in counts:
        metrics.append({"metric": f"openapi_linter.{severity[rule]}s", "points": [counts[rule]], "tags": [f"api:{api_name}", f"rule:{rule}"]})
...
def parse_exception_metrics(api_exceptions_filename, api_name):
    ...
    for rule, enabled in parsed.get("rules", {}).items():
            if not enabled:
                metrics.append({"metric": "openapi_linter.disabled_rules", "points": [1], "tags": [f"api:{api_name}", f"rule:{rule}"]})
    ...
    for rule in counts:
        metrics.append({"metric": "openapi_linter.exceptions", "points": [counts[rule]], "tags": [f"api:{api_name}", f"rule:{rule}"]})
...
def print_output(input):
    ...
    for violation in input:
        print ("{:<8} {:<8} {:<25} {:<20} {:<20}".format(format_range(violation['range']), severity_to_name[violation['severity']], violation['code'], violation['message'], format_path(violation['path'])))
        counts[violation['severity']] += 1
    
    summary = ", ".join([format_counts(counts[severity], name) for (severity, name) in severity_to_name.items()])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Report OpenAPI linting metrics to Datadog.")
    parser.add_argument("api_name", help="the short name of the API that metrics are associated with")
    parser.add_argument("api_spec_filename", help="path to the OpenAPI spec that was linted")
    
    # Since these arguments are optional, they need to be specified via flags
    parser.add_argument("--exceptions", "-e", help="path to a Spectral ruleset file defining linting exceptions, if any")
    parser.add_argument("--submit-metrics", action='store_true', help="enable posting metrics to datadog", default=os.environ.get('CI', False))
    parser.add_argument("--debug", action='store_true', help="enable debugging mode", default=os.environ.get('DEBUG', False))
    ...
    if (len(input) == 0 or not isFileChanged(args.api_spec_filename)) and not args.debug:
       return
    ...
    exception_metrics = []
    if args.exceptions:
        exception_metrics = parse_exception_metrics(args.exceptions, args.api_name)
