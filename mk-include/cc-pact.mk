...
RELEASE_POSTCOMMIT += pact-consumer-publish
...
# We're using `for` loop here to check if there's at least one .json file under $PACTS_DIR
...
ifeq ($(BRANCH_NAME),$(MASTER_BRANCH))
	if ls $(PACTS_DIR)/*.json &>/dev/null; then \
		...
		./pact/bin/pact-broker publish $(PACTS_DIR) \
			--consumer-app-version $(BUMPED_VERSION) \
			--branch $(BRANCH_NAME) \
	...
	@echo "--- not on $(MASTER_BRANCH) branch, skip publishing pacts"
endif
else
